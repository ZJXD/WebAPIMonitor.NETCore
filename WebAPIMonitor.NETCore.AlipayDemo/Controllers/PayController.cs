...
namespace WebAPIMonitor.NETCore.AlipayDemo.Controllers
{
    /// <summary>
    /// 支付控制器
    /// </summary>
    public class PayController : Controller
    {
        ...
        public void PayRequest(string tradeno, string subject, string totalAmout, string itemBody)
        {
            ...
            // 设置同步回调地址
            payRequest.SetReturnUrl($"https://{Request.Host}/Pay/Callback");
            // 设置异步通知接收地址
            payRequest.SetNotifyUrl("");
            // 将业务model载入到request
            payRequest.SetBizModel(payModel);

            var response = _alipayService.SdkExecute(payRequest);
